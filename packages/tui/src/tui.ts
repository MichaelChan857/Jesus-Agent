...
import { getEnvFlagWithFallback, getEnvWithFallback } from "./utils/env-with-fallback.ts";
...
export const VIEWPORT_TUI = Symbol.for("@jesus/tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private showHardwareCursor = getEnvFlagWithFallback("JESUS_HARDWARE_CURSOR", "PI_HARDWARE_CURSOR");
	private clearOnShrink = getEnvFlagWithFallback("JESUS_CLEAR_ON_SHRINK", "PI_CLEAR_ON_SHRINK");
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ??
			getEnvWithFallback("JESUS_CODING_AGENT_DIR", "PI_CODING_AGENT_DIR") ??
			path.join(os.homedir(), ".jesus", "agent");
