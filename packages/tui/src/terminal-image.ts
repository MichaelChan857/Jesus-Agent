...
import { getEnvWithFallback } from "./utils/env-with-fallback.ts";
...
export function detectCapabilities(tmuxForwardsHyperlink: () => boolean = probeTmuxHyperlinks): TerminalCapabilities {
	const hyperlinks = parseBooleanCapabilityOverride(getEnvWithFallback("JESUS_HYPERLINKS", "PI_HYPERLINKS"));
	const detected = detectCapabilitiesFromEnvironment(
		hyperlinks === undefined ? tmuxForwardsHyperlink : () => hyperlinks,
	);
	const imageProtocol = getEnvWithFallback("JESUS_IMAGE_PROTOCOL", "PI_IMAGE_PROTOCOL")?.toLowerCase();
	const images =
		imageProtocol === "kitty" || imageProtocol === "iterm2"
			...
			: imageProtocol === "none" || imageProtocol === "0"
				? null
				: undefined;
	const trueColor = parseBooleanCapabilityOverride(getEnvWithFallback("JESUS_TRUE_COLOR", "PI_TRUE_COLOR"));
	return {
		...detected,
