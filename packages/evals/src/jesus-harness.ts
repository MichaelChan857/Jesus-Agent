...
import { contentText } from "@jesus/ai";
import {
	type AgentSession,
...
export type JesusCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type JesusCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type JesusCodingAgentHarnessOptions = {
	name?: string;
	model?: JesusCodingAgentModelSelection;
	noTools?: CreateAgentSessionOptions["noTools"];
	transformSystemPrompt?: (defaultPrompt: string) => string;
};

type JesusCodingAgentHarnessWithOutput<TOutput extends JsonValue> = JesusCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: JesusCodingAgentModelSelection | undefined,
	environment: { PI_PROVIDER?: string; PI_MODEL?: string } = process.env,
): JesusCodingAgentModelSelection {
...
async function runJesusCodingAgent<TOutput extends JsonValue>(
	input: JesusCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: JesusCodingAgentHarnessOptions | JesusCodingAgentHarnessWithOutput<TOutput>,
...
export function createJesusCodingAgentHarness<TOutput extends JsonValue>(
	options: JesusCodingAgentHarnessWithOutput<TOutput>,
): Harness<JesusCodingAgentInput, TOutput>;
export function createJesusCodingAgentHarness(options?: JesusCodingAgentHarnessOptions): Harness<JesusCodingAgentInput, string>;
export function createJesusCodingAgentHarness<TOutput extends JsonValue>(
	options: JesusCodingAgentHarnessOptions | JesusCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<JesusCodingAgentInput, string | TOutput>({
		name: options.name ?? "pi-coding-agent",
		run: ({ input, signal, setArtifact }) => runJesusCodingAgent(input, signal, setArtifact, options),
	});
}
