...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@jesus/evals:session" &&
				artifact.type !== "@jesus/evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@jesus/evals:session" ? "sessions" : "sources";
