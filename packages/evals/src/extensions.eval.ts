...
import { createJesusCodingAgentHarness, type JesusCodingAgentInput } from "./jesus-harness.ts";
...
const ExtensionAuthoringJudge = createJudge<JesusCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@jesus/coding-agent")) {
				failures.push("extension does not import the canonical @jesus/coding-agent package");
			}
