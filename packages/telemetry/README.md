...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@jesus/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@jesus/telemetry';
...
`@jesus/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@jesus/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@jesus/agent-core';
...
- `@jesus/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@jesus/ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@jesus/agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
