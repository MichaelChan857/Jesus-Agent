...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@jesus/tui";
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			...
			cwd,
			contextFiles: [
				{ path: path.join(home, ".jesus", "agent", "AGENTS.md") },
	...
	test("shows system prompt context paths before project context files", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			systemPromptSource: { path: path.join(cwd, ".jesus", "SYSTEM.md") },
			appendSystemPromptSources: [{ path: path.join(cwd, ".jesus", "APPEND_SYSTEM.md") }],
