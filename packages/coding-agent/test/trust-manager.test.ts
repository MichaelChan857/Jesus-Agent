...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".jesus", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".jesus", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".jesus", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".jesus"), { recursive: true });
			writeFileSync(join(cwd, ".jesus", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".jesus"), { recursive: true, force: true });
