...
import { InMemoryModelsStore } from "@jesus/ai";
...
describe("Radius provider", () => {
	...
	it("fetches and stores the catalog for configured Radius auth", async () => {
		...
		const radiusRequest = vi
			.mocked(fetch)
			.mock.calls.find(([url]) => String(url) === "https://radius.jesus.dev/v1/config");
	...
	it("does not fetch or expose Radius models without configured auth", async () => {
		...
		expect(fetchSpy.mock.calls.some(([url]) => String(url).includes("radius.jesus.dev/v1/config"))).toBe(false);
	});
