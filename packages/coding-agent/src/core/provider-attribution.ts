import type { Api, Model, ProviderHeaders } from "@jesus/ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://jesus.dev",
			"X-OpenRouter-Title": "Jesus Agent",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Jesus Agent",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "Jesus Agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "Jesus Agent" };
}
