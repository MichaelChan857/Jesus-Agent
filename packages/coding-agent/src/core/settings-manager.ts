import type { ThinkingLevel } from "@jesus/agent-core";
import type { Transport } from "@jesus/ai";
import type { TuiMode as RendererTuiMode, ScrollViewScrollbar, TerminalCapabilities } from "@jesus/tui";
import { randomUUID } from "crypto";
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "fs";
import { dirname, join } from "path";
import lockfile from "proper-lockfile";
import { CONFIG_DIR_NAME, getAgentDir, getEnvWithFallback } from "../config.ts";
...
export interface Settings {
	...
	httpProxy?: string; // Proxy URL applied as HTTP_PROXY and HTTPS_PROXY for Jesus-managed HTTP clients
...
export class SettingsManager {
	...
	getClearOnShrink(): boolean {
		...
		return getEnvWithFallback("JESUS_CLEAR_ON_SHRINK", "PI_CLEAR_ON_SHRINK") === "1";
	}
	...
	getShowHardwareCursor(): boolean {
		return (
			this.settings.showHardwareCursor ?? getEnvWithFallback("JESUS_HARDWARE_CURSOR", "PI_HARDWARE_CURSOR") === "1"
		);
	}
