...
import { type ImageContent, modelsAreEqual } from "@jesus/ai";
import { setCapabilityOverrides } from "@jesus/tui";
...
import { AuthSetError, executeAuthSet, executeAuthUnset } from "./cli/auth-set.ts";
...
/**
 * Execute `auth set <provider> <key>` or `auth unset <provider>`.
 *
 * Validates that the provider exists in the model registry, then writes/clears the
 * credential in auth.json via AuthStorage. Network is disabled to keep this fast
 * and offline-safe; provider metadata is loaded from the bundled catalog.
 */
async function runAuthMutationCommand(command: AuthCommand): Promise<void> {
	if (command.kind !== "set" && command.kind !== "unset") {
		throw new AuthCommandError(`Unsupported mutation kind: ${command.kind}`);
	}

	let providerId: string;
	let key: string | undefined;
	if (command.kind === "set") {
		const parsed = parseAuthSetArgs(command.args);
		providerId = parsed.provider;
		key = parsed.key;
	} else {
		providerId = parseAuthUnsetArgs(command.args).provider;
	}
	...
	if (!provider) {
		throw new AuthCommandError(
			`Unknown provider "${providerId}". Run "${APP_NAME} --list-models" to see registered providers.`,
		);
	}

	if (command.kind === "unset") {
		const result = await executeAuthUnset({ providerId });
		process.stdout.write(
			result.removed
				? `Removed credential for provider "${providerId}".\n`
				: `No stored credential for provider "${providerId}".\n`,
		);
		return;
	}

	if (key === undefined) {
		throw new AuthCommandError(`auth set requires a key. Usage: ${APP_NAME} auth set <provider> <key>`);
	}
	const result = await executeAuthSet({ provider, providerId, key });
	process.stdout.write(
		result.created
			? `Saved API key for provider "${providerId}" (${provider.name}).\n`
			: `Updated API key for provider "${providerId}" (${provider.name}).\n`,
	);
}
...
async function runAuthCommand(args: string[]): Promise<boolean> {
	...
	// `auth set` and `auth unset` operate on positional arguments and write auth.json.
	// They bypass `parseArgs` to avoid swallowing the provider name as a message.
	if (command.kind === "set" || command.kind === "unset") {
		try {
			await runAuthMutationCommand(command);
		} catch (error) {
			const message =
				error instanceof AuthCommandError || error instanceof AuthSetError
					? error.message
					: `Failed to ${command.kind === "set" ? "set" : "unset"} credential`;
			console.error(chalk.red(`Error: ${message}`));
			process.exitCode = 1;
		}
		return true;
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || getEnvFlagWithFallback("JESUS_OFFLINE", "PI_OFFLINE");
	if (offlineMode) {
		process.env.JESUS_OFFLINE = "1";
		process.env.PI_OFFLINE = "1";
		process.env.JESUS_SKIP_VERSION_CHECK = "1";
	...
	const startupBenchmark = getEnvFlagWithFallback("JESUS_STARTUP_BENCHMARK", "PI_STARTUP_BENCHMARK");
