...
import { getEnvWithFallback } from "../config.ts";
import { getJesusUserAgent } from "./jesus-user-agent.ts";
import { fetchWithRetry } from "./management-http.ts";

const LATEST_VERSION_URL = "https://jesus.dev/api/latest-version";
...
export async function getLatestPiRelease(
	...
	if (getEnvWithFallback("JESUS_OFFLINE", "PI_OFFLINE")) return undefined;

	const response = await fetchWithRetry(
		LATEST_VERSION_URL,
		{
			headers: {
				"User-Agent": getJesusUserAgent(currentVersion),
...
export async function checkForNewPiVersion(currentVersion: string): Promise<LatestPiRelease | undefined> {
	if (getEnvWithFallback("JESUS_SKIP_VERSION_CHECK", "PI_SKIP_VERSION_CHECK")) return undefined;
