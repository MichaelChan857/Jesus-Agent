...
import { DEFAULT_RADIUS_GATEWAY } from "@jesus/ai/providers/radius-config";
import { type Container, type EditorComponent, hyperlink, type TUI } from "@jesus/tui";
...
async function tryShareViaRadius(tmpFile: string, context: SessionShareContext): Promise<boolean> {
	...
	try {
		...
		url.searchParams.set("title", "Jesus Agent session");
