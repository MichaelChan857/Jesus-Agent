 ...
 * set, matching how dual-named package.json fields (e.g. `appConfig` vs
 * `piConfig`) are resolved elsewhere.
 *
 * Pass `undefined` as `legacyName` to opt out of the fallback — most callers
 * want the fallback, but a few flags (debug-only) intentionally read only the
 * legacy name to avoid surprising users who opt into legacy behavior.
 */
...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return `Download from: https://github.com/jesus-agent/jesus-agent/releases/latest`;
	}
...
export function getPackageDir(): string {
	...
	const envDir = getEnvWithFallback("JESUS_PACKAGE_DIR", "PI_PACKAGE_DIR");
	if (envDir) {
...
// =============================================================================
// App Config (from package.json appConfig)
// =============================================================================
...
interface PackageJson {
	...
	/**
	 * @deprecated Kept for backward compatibility with forks of the original
	 * upstream. New code should publish `appConfig` instead. Reading order is
	 * `appConfig ?? piConfig`, so existing packages that only set `piConfig`
	 * continue to work but emit a one-time hint to migrate.
	 */
	piConfig?: {
		name?: string;
		configDir?: string;
...
const appConfig = pkg.appConfig ?? pkg.piConfig;
const appConfigName: string | undefined = appConfig?.name;
export const PACKAGE_NAME: string = pkg.name || "@jesus/coding-agent";
export const APP_NAME: string = appConfigName || "jesus";
export const APP_TITLE: string = appConfigName ? APP_NAME : "jesus";
...
const DEFAULT_SHARE_VIEWER_URL = "https://jesus.dev/session/";

/** Get the share viewer URL for a gist ID. */
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = getEnvWithFallback("JESUS_SHARE_VIEWER_URL", "PI_SHARE_VIEWER_URL") || DEFAULT_SHARE_VIEWER_URL;
	return `${baseUrl}#${gistId}`;
}

// =============================================================================
// User Config Paths (~/.jesus/agent/*)
// =============================================================================

/** Get the agent config directory (e.g., ~/.jesus/agent/) */
