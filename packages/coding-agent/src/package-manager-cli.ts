...
import { Markdown, type MarkdownTheme } from "@jesus/tui";
...
const DEFAULT_INSTALLER_API_BASE = "https://jesus.dev/api/installer/releases";
...
function getActiveManagedInstallRoot(): string | undefined {
	const configuredRoot = getEnvWithFallback("JESUS_MANAGED_INSTALL_ROOT", "PI_MANAGED_INSTALL_ROOT")?.trim();
	...
	// source checkout or another Jesus installation launched from managed Jesus as managed.
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getJesusUserAgent(VERSION) } });
	if (!response.ok) {
...
function verifyManagedRelease(releaseDir: string, expectedVersion: string): void {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(`Could not verify managed Jesus ${expectedVersion}: ${reason}`);
	}
	const installedVersion = result.stdout.trim();
	if (installedVersion !== expectedVersion) {
		throw new Error(`Managed Jesus smoke test returned version ${installedVersion}; expected ${expectedVersion}.`);
	}
}
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ELOCKED") {
			throw new Error("Another managed Jesus update is already running.");
		}
		throw error;
	}
	...
	try {
		...
		const installerApiBase = (
			getEnvWithFallback("JESUS_INSTALLER_API_BASE", "PI_INSTALLER_API_BASE")?.trim() || DEFAULT_INSTALLER_API_BASE
		).replace(/\/+$/, "");
