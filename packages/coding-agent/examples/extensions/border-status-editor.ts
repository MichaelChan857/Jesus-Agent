import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@jesus/coding-agent";
import type { Component, EditorTheme, TUI } from "@jesus/tui";
...
export default function (jesus: ExtensionAPI) {
	...
	jesus.on("agent_settled", () => {
		isWorking = false;
		stopSpinner();
		activeTui?.requestRender();
	});

	jesus.on("session_shutdown", () => {
		stopSpinner();
		activeTui = undefined;
	});
	...
	jesus.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await jesus.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = jesus.getThinkingLevel();
