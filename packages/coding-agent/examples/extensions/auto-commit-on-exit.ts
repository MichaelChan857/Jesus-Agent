...
export default function (jesus: ExtensionAPI) {
	jesus.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await jesus.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[jesus] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await jesus.exec("git", ["add", "-A"]);
		const { code: commitCode } = await jesus.exec("git", ["commit", "-m", commitMessage]);
