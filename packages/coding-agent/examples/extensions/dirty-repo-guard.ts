...
async function checkDirtyRepo(
	...
	const { stdout, code } = await jesus.exec("git", ["status", "--porcelain"]);
...
export default function (jesus: ExtensionAPI) {
	jesus.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(jesus, ctx, action);
	});

	jesus.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(jesus, ctx, "fork");
	});
}
