...
export default function (jesus: ExtensionAPI) {
	...
	jesus.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "jesus RPC Demo (new session)" : "jesus RPC Demo");
	...
	jesus.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	jesus.on("turn_end", async (_event, ctx) => {
	...
	jesus.registerCommand("rpc-input", {
	...
	jesus.registerCommand("rpc-editor", {
	...
	jesus.registerCommand("rpc-prefill", {
