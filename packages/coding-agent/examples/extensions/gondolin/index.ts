 ...
 *
 * Runs jesus's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   jesus -e /path/to/jesus/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (jesus: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `jesus ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	jesus.on("session_shutdown", async (_event, ctx) => {
		const activeVm = vm;
		vm = undefined;
		vmStarting = undefined;
		if (!activeVm) return;
	...
	jesus.registerCommand("gondolin", {
	...
	jesus.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
