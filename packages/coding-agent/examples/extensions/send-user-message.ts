 ...
 *
 * Demonstrates jesus.sendUserMessage() for sending user messages from extensions.
 * Unlike jesus.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (jesus: ExtensionAPI) {
	...
	jesus.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			jesus.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	jesus.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				jesus.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	jesus.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				jesus.sendUserMessage(args, { deliverAs: "followUp" });
