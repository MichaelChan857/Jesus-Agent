 ...
 *
 *   mkdir -p ~/.jesus/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.jesus/agent/extensions/
 *
 * Or:
 *
 *   jesus -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .jesus, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@jesus/coding-agent";
...
export default function (jesus: ExtensionAPI) {
	...
	jesus.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
