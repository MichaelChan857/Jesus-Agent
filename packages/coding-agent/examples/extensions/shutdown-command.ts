 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit jesus cleanly.
 */
...
export default function (jesus: ExtensionAPI) {
	// Register a /quit command that cleanly exits jesus
	jesus.registerCommand("quit", {
		description: "Exit jesus cleanly",
	...
	jesus.registerTool({
		...
		description: "Complete a task and exit jesus",
	...
	jesus.registerTool({
		...
		description: "Deploy the application and exit jesus",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await jesus.exec("npm", ["run", "deploy", params.environment], { signal });
