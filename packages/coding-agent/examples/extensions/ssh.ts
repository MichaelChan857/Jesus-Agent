 ...
 *
 * Usage:
 *   jesus -e ./ssh.ts --ssh user@host
 *   jesus -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (jesus: ExtensionAPI) {
	jesus.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	jesus.on("session_start", async (_event, ctx) => {
		...
		const arg = jesus.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
