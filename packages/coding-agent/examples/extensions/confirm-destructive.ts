...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@jesus/coding-agent";

export default function (jesus: ExtensionAPI) {
	jesus.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
