...
async function resolveGitHubRepo(jesus: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await jesus.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (jesus: ExtensionAPI): void {
	jesus.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(jesus, ctx.cwd);
		if (!resolvedRepo.ok) {
