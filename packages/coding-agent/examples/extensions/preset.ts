 ...
 * - ~/.jesus/agent/presets.json (global)
 * - <cwd>/.jesus/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `jesus --preset plan` - start with plan preset
...
import type { Api, Model } from "@jesus/ai";
import type { ExtensionAPI, ExtensionContext } from "@jesus/coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@jesus/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@jesus/tui";
...
export default function presetExtension(jesus: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: jesus.getThinkingLevel(),
				tools: jesus.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			jesus.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = jesus.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				jesus.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await jesus.setModel(originalState.model);
				}
				jesus.setThinkingLevel(originalState.thinkingLevel);
				jesus.setActiveTools(originalState.tools);
			} else {
				jesus.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	jesus.registerShortcut(Key.ctrlShift("u"), {
	...
	jesus.on("before_agent_start", async (event) => {
	...
	jesus.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = jesus.getFlag("preset");
	...
	jesus.on("turn_start", async () => {
		if (activePresetName) {
			jesus.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
