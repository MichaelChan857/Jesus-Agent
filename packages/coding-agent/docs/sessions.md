...
Sessions auto-save to `~/.jesus/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
jesus -c                  # Continue most recent session
jesus -r                  # Browse and select from past sessions
jesus --no-session        # Ephemeral mode; do not save
jesus --name "my task"    # Set session display name at startup
jesus --session <path|id> # Use a specific session file or partial session ID
jesus --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `jesus -r` opens the same picker at startup.
...
When available, jesus uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
jesus --name "Refactor auth module"
jesus --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `jesus -r`.
...
When `/tree` switches away from one branch to another, jesus can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
