...
Discovery rules:
- In `~/.jesus/agent/skills/` and `.jesus/skills/`, direct root `.md` files are discovered as individual skills when they have valid skill frontmatter with a non-empty `description`
...
For project-level Claude Code skills, add to `.jesus/settings.json`:
