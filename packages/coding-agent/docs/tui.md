...
**Source:** [`@jesus/tui`](https://github.com/jesus-agent/jesus-agent/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@jesus/tui";
...
```typescript
import { Container, type Focusable, Input } from "@jesus/tui";
...
Import from `@jesus/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@jesus/tui";
```
...
```typescript
import { matchesKey, Key } from "@jesus/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@jesus/tui";
...
```typescript
import { getMarkdownTheme } from "@jesus/coding-agent";
import { Markdown } from "@jesus/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@jesus/tui` with `DynamicBorder` for framing.
...
For toggling multiple settings. Use `SettingsList` from `@jesus/tui` with `getSettingsListTheme()`.
...
import { matchesKey, truncateToWidth } from "@jesus/tui";
