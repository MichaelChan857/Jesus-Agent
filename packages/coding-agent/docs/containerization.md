...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.jesus/agent/extensions/gondolin
cd ~/.jesus/agent/extensions/gondolin
...
docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v jesus-agent-home:/root/.jesus/agent \
  pi-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.jesus/agent` if you want container-local settings and sessions. Mounting your host `~/.jesus/agent` exposes host auth and session files to the container.
