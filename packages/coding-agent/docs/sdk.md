> jesus can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to jesus's agent capabilities. Use it to embed jesus in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @jesus/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@jesus/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `jesus.sendMessage()`.
...
The `Agent` class (from `@jesus/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.jesus/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.jesus/extensions/`)
- Project skills:
  - `.jesus/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.jesus/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.jesus/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@jesus/ai";
import { ModelRuntime } from "@jesus/coding-agent";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from jesus.dev by default.
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.jesus/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `PI_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@jesus/ai";
import { createAgentSession, ModelRuntime } from "@jesus/coding-agent";

// Default: uses ~/.jesus/agent/auth.json and ~/.jesus/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@jesus/coding-agent";
...
```typescript
import { createAgentSession } from "@jesus/coding-agent";
...
import { createAgentSession, defineTool } from "@jesus/coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `jesus.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `jesus.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.jesus/agent/extensions/`, `.jesus/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@jesus/coding-agent";
...
**Event Bus:** Extensions can communicate via `jesus.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@jesus/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@jesus/coding-agent";
...
1. Global: `~/.jesus/agent/settings.json`
2. Project: `<cwd>/.jesus/settings.json`
