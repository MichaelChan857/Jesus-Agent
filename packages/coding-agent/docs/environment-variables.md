...
- `AI_AGENT=jesus` is a generic marker that lets tooling identify Pi as the agent that launched the process.
...
| Variable | Description |
|----------|-------------|
| `PI_CODING_AGENT_DIR` | Override the config directory; default is `~/.jesus/agent` |
...
| `PI_SKIP_VERSION_CHECK` | Disable the `jesus.dev` latest-version request |
