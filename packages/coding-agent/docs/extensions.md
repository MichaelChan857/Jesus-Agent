> jesus can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend jesus's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.jesus/agent/extensions/` (global) or `.jesus/extensions/` (project-local) for auto-discovery. Use `jesus -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `jesus.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `jesus.registerCommand()`
- **Session persistence** - Store state that survives restarts via `jesus.appendEntry()`
...
Create `~/.jesus/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.jesus/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.jesus/agent/extensions/*.ts` | Global (all projects) |
| `~/.jesus/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.jesus/extensions/*.ts` | Project-local |
| `.jesus/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as jesus packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@jesus/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@jesus/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@jesus/tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed jesus packages installed with `jesus install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (jesus: ExtensionAPI) {
  // Subscribe to events
  jesus.on("event_name", async (event, ctx) => {
  ...
  jesus.registerTool({ ... });
  jesus.registerCommand("name", { ... });
  jesus.registerShortcut("ctrl+x", { ... });
  jesus.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, jesus awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `jesus.registerProvider()` are flushed.
...
This pattern makes the fetched models available during normal startup and to `jesus --list-models`.
...
thinking level changes (settings, keybinding, jesus.setThinkingLevel())
...
Fired before jesus decides whether to trust a project with dynamic configs (`.jesus` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
jesus.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether jesus asks, trusts, or declines by default.
...
```typescript
jesus.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
jesus.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `jesus.setSessionName()`.

```typescript
jesus.on("session_info_changed", async (event, ctx) => {
...
```typescript
jesus.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, jesus emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
jesus.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, jesus emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
jesus.on("session_compact", async (event, ctx) => {
...
jesus.on("session_tree", async (event, ctx) => {
...
```typescript
jesus.on("session_shutdown", async (event, ctx) => {
...
```typescript
jesus.on("before_agent_start", async (event, ctx) => {
...
```typescript
jesus.on("agent_start", async (_event, ctx) => {});

jesus.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

jesus.on("agent_settled", async (_event, ctx) => {
...
```typescript
jesus.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

jesus.on("turn_end", async (event, ctx) => {
...
```typescript
jesus.on("message_start", async (event, ctx) => {
  // event.message
});

jesus.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

jesus.on("message_end", async (event, ctx) => {
...
```typescript
jesus.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

jesus.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

jesus.on("tool_execution_end", async (event, ctx) => {
...
```typescript
jesus.on("context", async (event, ctx) => {
...
```typescript
jesus.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header jesus adds for this call
...
```typescript
jesus.on("before_provider_request", (event, ctx) => {
...
```typescript
jesus.on("after_provider_response", (event, ctx) => {
...
```typescript
jesus.on("model_select", async (event, ctx) => {
...
```typescript
jesus.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `jesus.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, jesus waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@jesus/coding-agent";
...
```typescript
import { isBashToolResult } from "@jesus/coding-agent";

jesus.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@jesus/coding-agent";

jesus.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap jesus's built-in local bash backend
...
```typescript
jesus.on("input", async (event, ctx) => {
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.jesus` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@jesus/coding-agent";
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while jesus is idle.
...
Request a graceful shutdown of jesus.
...
```typescript
jesus.on("before_agent_start", (event, ctx) => {
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `jesus` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@jesus/coding-agent";

jesus.registerCommand("switch", {
...
- Captured old `jesus` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
jesus.registerCommand("reload-runtime", {
...
export default function (jesus: ExtensionAPI) {
  ...
  jesus.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      jesus.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### jesus.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### jesus.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`jesus.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `jesus.getAllTools()` and are callable by the LLM without `/reload`.

Use `jesus.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@jesus/ai";
...
jesus.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    // Stream progress
    onUpdate?.({ content: [{ type: "text", text: "Reasoning..." }] });
...
### jesus.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`jesus.appendEntry()`](#piappendentrycustomtype-data) with [`jesus.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### jesus.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
jesus.sendUserMessage("What is 2+2?");

// With content array (text + images)
jesus.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
jesus.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
jesus.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
jesus.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### jesus.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `jesus.registerEntryRenderer()`.

```typescript
jesus.appendEntry("my-state", { count: 42 });
jesus.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### jesus.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
jesus.setSessionName("Refactor auth module");
```

### jesus.getSessionName()

Get the current session name, if set.

```typescript
const name = jesus.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### jesus.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
jesus.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
jesus.setLabel(entryId, undefined);
...
### jesus.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, jesus keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
jesus.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@jesus/tui";

jesus.registerCommand("deploy", {
...
```typescript
const commands = jesus.getCommands();
...
### jesus.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `jesus.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### jesus.registerMarkdownTransformer(transformer)
...
```typescript
jesus.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
...
### jesus.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `jesus.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@jesus/tui";

jesus.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### jesus.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
jesus.registerShortcut("ctrl+shift+p", {
...
### jesus.registerFlag(name, options)

Register a CLI flag.

```typescript
jesus.registerFlag("plan", {
...
// Check value
if (jesus.getFlag("plan")) {
  // Plan mode enabled
}
```

### jesus.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await jesus.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### jesus.getActiveTools() / jesus.getAllTools() / jesus.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `jesus.getActiveTools()` returns the active tool names as `string[]`; `jesus.getAllTools()` returns metadata for all configured tools.

```typescript
const active = jesus.getActiveTools(); // ["read", "bash", ...]
const all = jesus.getAllTools();
// all = [{
//   name: "read",
...
jesus.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
jesus.setActiveTools(["read", "bash"]); // Switch to read-only
```

`jesus.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await jesus.setModel(model);
  if (!success) {
...
### jesus.getThinkingLevel() / jesus.setThinkingLevel(level)
...
```typescript
const current = jesus.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
jesus.setThinkingLevel("high");
```
...
```typescript
jesus.events.on("my:event", (data) => { ... });
jesus.events.emit("my:event", { ... });
```

### jesus.registerProvider(name, config)
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@jesus/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.
...
jesus.registerProvider(provider);

// Register a new provider with custom models
jesus.registerProvider("my-proxy", {
  name: "My Proxy",
...
jesus.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
```typescript
jesus.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    jesus.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `jesus.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `jesus.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@jesus/coding-agent";
...
jesus.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via jesus.exec (captured from extension closure)
    const result = await jesus.exec("some-command", [], { signal });
...
**Important:** Use `StringEnum` from `@jesus/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when jesus resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
jesus -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
jesus --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@jesus/coding-agent";
...
For `user_bash`, extensions can reuse jesus's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The `bash` and `powershell` tools also support a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@jesus/coding-agent";
...
export default function (jesus: ExtensionAPI) {
  ...
  jesus.registerTool({ name: "db_connect", ... });
  jesus.registerTool({ name: "db_query", ... });
  jesus.registerTool({ name: "db_close", ... });

  jesus.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/jesus-agent/jesus-agent/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@jesus/coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `jesus.setActiveTools()` during execution. Pi detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `jesus.registerTool()` so it appears in `jesus.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `jesus.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
...
You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `jesus.setActiveTools()` must already be registered; unknown names are ignored.
...
export default function (jesus: ExtensionAPI) {
  ...
  jesus.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = jesus.getAllTools()
      ...
      jesus.setActiveTools([...new Set([...active, ...added])]);
  ...
  jesus.on("session_start", () => {
    ...
    const initialTools = jesus.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    jesus.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("jesus - my-project");
...
```typescript
import { Text, Component } from "@jesus/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@jesus/coding-agent";
import { matchesKey } from "@jesus/tui";
...
jesus.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `jesus.sendMessage()`:
...
```typescript
jesus.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

jesus.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@jesus/coding-agent";
...
| `event-bus.ts` | Inter-extension events | `jesus.events` |
| **Session Metadata** |||
