...
```bash
npm install @jesus/ai
```

TypeBox exports are re-exported from `@jesus/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@jesus/ai';
import { builtinModels } from '@jesus/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@jesus/ai/providers/anthropic';
import { openaiProvider } from '@jesus/ai/providers/openai';
import { openrouterProvider } from '@jesus/ai/providers/openrouter';
import { amazonBedrockProvider } from '@jesus/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@jesus/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@jesus/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@jesus/ai';
...
```typescript
import { validateToolCall, type Tool } from '@jesus/ai';
...
```typescript
import { builtinImagesModels } from '@jesus/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@jesus/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@jesus/ai';
import { openAICompletionsApi } from '@jesus/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@jesus/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@jesus/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@jesus/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@jesus/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@jesus/ai';
import { anthropicProvider } from '@jesus/ai/providers/anthropic';
import { openaiProvider } from '@jesus/ai/providers/openai';
import { googleProvider } from '@jesus/ai/providers/google';
...
```typescript
import { createModels } from '@jesus/ai';
import { anthropicProvider } from '@jesus/ai/providers/anthropic';
...
- `@jesus/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@jesus/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@jesus/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@jesus/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@jesus/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@jesus/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@jesus/ai/bedrock-provider';
...
```bash
npx @jesus/ai login              # interactive provider selection
npx @jesus/ai login anthropic    # login to specific provider
npx @jesus/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@jesus/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@jesus/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@jesus/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@jesus/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@jesus/ai`.
