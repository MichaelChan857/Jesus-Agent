...
export interface AppMessagesOptions extends StreamOptions {
...
type AppMessagesUsage = AssistantMessage["usage"];
type AppMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type AppMessagesRewriteImpact = {
	policyId: string;
...
export type AppMessagesEvent =
	...
	| {
			type: "done";
			reason: Extract<AppMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: AppMessagesUsage;
			responseId?: string;
			rewrite?: AppMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<AppMessagesStopReason, "aborted" | "error">;
			usage: AppMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			rewrite?: AppMessagesRewriteImpact;
...
export class AppMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "AppMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parseAppMessagesErrorBody(body: string): AppMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as AppMessagesErrorBody | null;
...
function formatAppMessagesResponseError(
	response: Response,
	body: string,
	errorBody: AppMessagesErrorBody | undefined,
): string {
...
function createAppMessagesResponseError(
	model: Model<"pi-messages">,
	url: URL,
	response: Response,
	body: string,
): AppMessagesResponseError {
	const errorBody = parseAppMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new AppMessagesResponseError(formatAppMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): AppMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: AppMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
...
function createEventConverter(model: Model<"pi-messages">) {
	...
	return (event: AppMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readAppMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<AppMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseAppMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseAppMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseAppMessagesEvent(raw: string): AppMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as AppMessagesEvent) : undefined;
}
...
function createErrorEvent(model: Model<"pi-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof AppMessagesResponseError) {
...
export const stream: StreamFunction<"pi-messages", AppMessagesOptions> = (
	model: Model<"pi-messages">,
	context: Context,
	options?: AppMessagesOptions,
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createAppMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const appEvent of readAppMessagesEvents(response.body)) {
				const event = convertEvent(appEvent);
...
export const streamSimple: StreamFunction<"pi-messages", SimpleStreamOptions> = (
	...
	const extra = options as AppMessagesOptions | undefined;
