...
Stateful agent with tool execution and event streaming. Built on `@jesus/ai`.

## Installation

```bash
npm install @jesus/agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@earendil-works/pi-session-backends-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.

## Quick Start

```typescript
import { Agent } from "@jesus/agent-core";
...
```typescript
declare module "@jesus/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@jesus/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@jesus/agent-core";
