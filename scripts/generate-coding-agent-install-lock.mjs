...
// Internal workspace packages — Jesus core 5 packages plus side packages
// retained under the upstream @earendil-works/pi-* scope.
const internalPackagePrefixes = ["@jesus/", "@earendil-works/pi-"];
const internalPackagePrefix = internalPackagePrefixes.join("|");
const installPackageName = "@jesus/coding-agent-install";
...
function getInternalWorkspaces(lockPackages) {
	...
	for (const [lockPath, entry] of Object.entries(lockPackages)) {
		...
		if (!internalPackagePrefixes.some(p => entry.name.startsWith(p))) {
			continue;
		}
...
function resolveExternalDependency(lockPackages, packageName, fromLockPath) {
	...
	const tried = new Set();
	// First pass: skip hoisted root-level entries that are dev-only (transitive deps of other workspaces).
	// Dev entries are not part of the installer's runtime dep graph, so prefer a non-dev match even if it is
	// nested deeper in the lockfile. Fall back to the dev entry if no non-dev match exists.
...
function validateGeneratedFiles(installerPackageJson, installLock, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(installLock.packages)) {
		...
		if (packageName && internalPackagePrefixes.some(p => packageName.startsWith(p)) && entry.version !== installerPackageJson.version) {
