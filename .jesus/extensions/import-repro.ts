 ...
 *   /ir https://jesus.dev/session/#b4d100022aefb12f25dd2d8485e0a82a
 *   /ir https://github.com/jesus-agent/jesus-agent/issues/123
 *
 *   pi "/ir <gist-id>"
 */
...
import type { ExtensionAPI, ExtensionCommandContext } from "@jesus/coding-agent";
...
function parseRef(
	...
	throw new Error(`expected a gist ID, gist URL, jesus.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`);
}
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("ir", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			const ref = args.trim();
			if (!ref) {
				ctx.ui.notify("Usage: /ir <gist-id | gist-url | jesus.dev/session URL | issue URL>", "error");
				return;
			}
